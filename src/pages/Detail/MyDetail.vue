                ...
                <div class="InfoWrap">
                    ...
                    <div class="choose">
                        ...
                        <div class="cartWrap">
                            ...
                            <div class="add">
                                <!-- 
                                    之前的路由跳转是从A路由跳转到B路由。而这里产品加入购物车操作，是在进行路由跳转之前，发
                                    请求把你购买的产品的信息通过请求的形式通知服务器，服务器进行相应的存储
                                 -->
...
<script>
    ...
    import MyZoom from './Zoom/MyZoom'
    ...
    export default {
        ...
        computed: {
            ...
            // 给子组件的数据
            skuImageList() {
                // 如果服务器数据没有返回回来，skuInfo这个数组是个空数组
                return this.skuInfo.skuImageList || []
            }
        },
        methods: {
            // 商品的售卖属性值切换高亮（通过修改isChecked来实现）--- 排它
            changeActive(spuSaleAttrValue, arr) {
                // spuSaleAttrValue代表的是当前点击的是哪个商品售卖属性值
                // arr代表的是商品售卖属性值的数组
                // 遍历全部的售卖属性值isChecked并使其为零 --- 没有高亮
                arr.forEach(item => {
                    item.isChecked = 0
                })
                // 再将点击的那个售卖属性值的isChecked变为1
            ...
            changeSkuNum(event) {
                ...
                // 如果用户输入进来的是非法的（非法：出现NaN或者小于1）
                if(isNaN(value) || value < 1) {
                    this.skuNum = 1
                }else {
                    // 正常且大于1的（但要注意的是：会出现小数，所以需要使用paseInt将其转换为整数）
            ...
            async addShopCart() {
                ...
                /* 
                    当前这里是派发一个action，也向服务器发请求，判断加入购物车是成功还是失败了，再进行相应的操作。
                    this.$store.dispatch('addOrUpdateShopCart', { skuId: this.$route.params.skuid, skuNum: this.skuNum })
                    上面这行代码说白了：就是调用仓库中的addOrUpdateShopCart，这个方法加上asyc，返回的一定是一个Promise
                    返回的结果：要么成功；要么失败。
                */
                try {
                    ...
                    // 路由跳转：在进行路由跳转的时候还需要将产品的信息带给下一级的路由组件
                    // 一些简单的数据skuNum，通过query形式给路由组件传递过去
                    // 而一些比较复杂的数据skuInfo（产品信息的数据），通过会话存储（不持久化，会话结束后数据就消失）传递过去
